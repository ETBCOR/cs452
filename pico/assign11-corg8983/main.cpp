 ...
 * Project:     Assignment 11 - Pixel Magic          *
 * File:        main.c                               *
 *                                                   *
\* * * * * * * * * * * * * * * * * * * * * * * * * * */
...
#define RGB(R, G, B) PicoLed::RGBW(G, R, B, 0)
...
static TaskHandle_t xPixelTsk          = NULL;
...
static QueueHandle_t xPixelQueue       = NULL;
...
// This enum represents main-state of the program
...
static enum MovementStateEnum mvmtState = MV_ST_IDLE;

// This enum represents what this display should show
...
static enum DisplayStateEnum dsplState = DS_ST_IDLE;

// This enum controls whether the display uses dec or hexidec
enum DisplayModeEnum {
	DM_DEC,
	DM_HEX
};
static enum DisplayModeEnum dsplMode = DM_DEC;
...
typedef struct PixelMsgStruct {
	PixelMsgEnum color;
	int pixelIdx;
} PixelMsg;


// Struct for button messages
...
// Struct for reading from the HDC1080
...
struct PixelParametersStruct {
	PicoLed::PicoLedController * pixels;
};
...
int main()
{
	...
	PicoLed::PicoLedController pixels = PicoLed::addLeds<PicoLed::WS2812B>(pio0, 0, PIXEL_PIN, PIXEL_LEN, PicoLed::FORMAT_WRGB);
	pixels.setBrightness(16);

	PixelParametersStruct pxlPrms = { &pixels };
	...
	xPixelQueue = xQueueCreate(10, sizeof(PixelMsg));
	...
	// Create the tasks
	/*xTaskCreate((TaskFunction_t)vMainTsk,             "Main_Task",            	256, NULL, 1, &xMainTsk);
	xTaskCreate((TaskFunction_t)vButtonTsk,           "Button_Task",            256, NULL, 6, &xButtonTsk);
	xTaskCreate((TaskFunction_t)vSwitcherTsk,         "Switcher_Task",          256, NULL, 3, &xSwitcherTsk);
	xTaskCreate((TaskFunction_t)vSevSegTsk,           "Sev_Seg_Task",           256, NULL, 5, &xSevSegTsk);
	xTaskCreate((TaskFunction_t)vHDC1080ConnectorTsk, "HDC1080_Connector_Task", 256, NULL, 7, &xHDC1080CScanTsk);
	xTaskCreate((TaskFunction_t)vHDC1080Tsk,          "HDC1080_Task",           256, NULL, 2, &xHDC1080Tsk);
	xTaskCreate((TaskFunction_t)vStepperTsk,          "Stepper_Task",           256, NULL, 4, &xStepperTsk);
 // xTaskCreate((TaskFunction_t)vPixelTsk,            "Pixel_Task",             256, NULL, 3, &xPixelTsk);
	xTaskCreate((TaskFunction_t)vPixelTsk,            "Pixel_Task",             256, &pxlPrms, 3, &xPixelTsk);*/
	
	xTaskCreate((TaskFunction_t)vHDC1080ConnectorTsk, "HDC1080_Connector_Task", 256, NULL, 8, &xHDC1080CScanTsk);
	xTaskCreate((TaskFunction_t)vButtonTsk,           "Button_Task",            256, NULL, 7, &xButtonTsk);
	xTaskCreate((TaskFunction_t)vSevSegTsk,           "Sev_Seg_Task",           256, NULL, 6, &xSevSegTsk);
	xTaskCreate((TaskFunction_t)vStepperTsk,          "Stepper_Task",           256, NULL, 5, &xStepperTsk);
	xTaskCreate((TaskFunction_t)vPixelTsk,            "Pixel_Task",             256, &pxlPrms, 4, &xPixelTsk);
	xTaskCreate((TaskFunction_t)vSwitcherTsk,         "Switcher_Task",          256, NULL, 3, &xSwitcherTsk);
	xTaskCreate((TaskFunction_t)vHDC1080Tsk,          "HDC1080_Task",           256, NULL, 2, &xHDC1080Tsk);
	xTaskCreate((TaskFunction_t)vMainTsk,             "Main_Task",            	256, NULL, 1, &xMainTsk);
...
// Controls the switching of modes and sending of most messages
static void vSwitcherTsk()
{
	...
	while (1) {
		...
		switch (pressCount) {
			...
			case 1: // one press
				...
				switch (bm1.pin) {
					...
					case BTN1:
						...
						pxlMsg.pixelIdx = 2;
						xQueueSend(xPixelQueue, &pxlMsg, 0);
			...
			case 2: // two presses
				...
				pxlMsg.color = PixelMsgEnum::GREEN;
			...
			case 3: // three presses

				pxlMsg.color = PixelMsgEnum::BLUE;
				...
				switch (bm1.pin) {
					...
					case BTN1:
						...
						pxlMsg.color = PixelMsgEnum::RESET;
						xQueueReset(xPixelQueue);
						xQueueSend(xPixelQueue, &pxlMsg, 0);
			...
			case 4: // four presses
				if (bm1.pin == BTN1) {
					...
					if (dsplMode == DM_DEC) dsplMode = DM_HEX;
					else dsplMode = DM_DEC;
					printf("[Switcher_Task]: Switch display mode to %s.\n", dsplMode ? "hexadecimal" : "decimal");

					pxlMsg.color = (dsplMode == DM_DEC) ? PixelMsgEnum::RED : PixelMsgEnum::GREEN;
					pxlMsg.pixelIdx = 1;
					xQueueSend(xPixelQueue, &pxlMsg, 0);
					xQueueSend(xSevSegQueue, &msg, 0);
					...
					xSemaphoreGive(xDsplModeSem);
				} else if (bm1.pin = BTN3) {
					printf("[Switcher_Task]: Switch LEDs to rainbow mode.\n");

					pxlMsg.color = PixelMsgEnum::RAINBOW;
					xQueueSend(xPixelQueue, &pxlMsg, 0);
				}
				break;
...
static void vHDC1080ConnectorTsk()
{
	...
	while (1) {
		...
		while (!i2c_connect_HDC1080()) {
			...
			sleep_ms(1000); // use sleep to intentionally block all other tasks until the sensor is reconnected
...
static void vStepperTsk()
{
	...
	while (1) {
		...
		if (idle) {
			gpio_put(STEPPER_PIN_1, 0);
			gpio_put(STEPPER_PIN_2, 0);
			gpio_put(STEPPER_PIN_3, 0);
			gpio_put(STEPPER_PIN_4, 0);
		} else {
...
static void vPixelTsk(void * pxlPrms)
{
	PixelParametersStruct * pxlPrms_ = (PixelParametersStruct *)pxlPrms;
	PicoLed::PicoLedController * pixels = pxlPrms_->pixels;
	PixelMsg msg;

	pixels->setPixelColor(1, RGB(255, 0, 0));
	pixels->show();
	...
	while (1) {
		...
		// Block until a message is received
		xQueueReceive(xPixelQueue, &msg, portMAX_DELAY);

		if (msg.pixelIdx == 2) pixels->setPixelColor(3, RGB(0, 0, 0));
		else if (msg.pixelIdx == 3) pixels->setPixelColor(2, RGB(0, 0, 0));

		switch (msg.color) {
			case RESET:
				printf("[Pixel_Task]: Resetting the LEDs.\n");
				pixels->clear();
				pixels->setPixelColor(1, RGB(255, 0, 0));
				break;

			case RED:
				printf("[Pixel_Task]: Setting LED #%i to red.\n", msg.pixelIdx + 1);
				pixels->setPixelColor(msg.pixelIdx, RGB(255, 0, 0));
				break;

			case GREEN:
				printf("[Pixel_Task]: Setting LED #%i to green.\n", msg.pixelIdx + 1);
				pixels->setPixelColor(msg.pixelIdx, RGB(0, 255, 0));
				break;

			case BLUE:
				printf("[Pixel_Task]: Setting LED #%i to blue.\n", msg.pixelIdx + 1);
				pixels->setPixelColor(msg.pixelIdx, RGB(0, 0, 255));
				break;

			case RAINBOW:
				printf("[Pixel_Task]: Setting LEDs to rainbow mode.\n");
				while (1) {
					if(xQueuePeek(xPixelQueue, &msg, 0) == errQUEUE_EMPTY) {
						hue += 1;
						if (hue >= 255) hue -= 255; 
						pixels->fillRainbow(hue, hue + 5);
						pixels->show();
			...
			default: printf("[Pixel_Task]: Unknownk pixel message.\n");
		}
		pixels->show();
	}
}